...
const getSellerDiagnostics = async (seller) => {
    try {
        // Priority 1: Admin-defined diagnostics
        if (seller.store_diagnostics && (
            seller.store_diagnostics.fulfillment || 
            seller.store_diagnostics.rating || 
            seller.store_diagnostics.responseTime || 
            seller.store_diagnostics.qualityScore
        )) {
            const diag = seller.store_diagnostics;
            return {
                fulfillment: diag.fulfillment ? (diag.fulfillment.includes('%') ? diag.fulfillment : `${diag.fulfillment}%`) : '100%',
                rating: diag.rating ? (diag.rating.includes('/') ? diag.rating : `${diag.rating}/5`) : '4.5/5',
                responseTime: diag.responseTime || '< 2 Hours',
                qualityScore: diag.qualityScore ? (diag.qualityScore.includes('%') ? diag.qualityScore : `${diag.qualityScore}%`) : '95%'
            };
        }

        // Priority 2: Calculated diagnostics (fallback)
